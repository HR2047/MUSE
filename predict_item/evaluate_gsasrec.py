...
device = get_device()
model = build_model_label(config, num_items)
model = model.to(device)
model.load_state_dict(torch.load(args.checkpoint, map_location=device))

# test_dataloader = get_dataloader(config.dataset_name, batch_size=config.eval_batch_size, max_length=config.sequence_length)
