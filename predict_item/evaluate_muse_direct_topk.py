...
def main():
    ...
    pparser.add_argument('--filter_rated',type=lambda x: x.lower() in ("true", "1", "yes"),default=True)
    ...
    with torch.no_grad():
        ...
        for batch_idx, (pos_emb, target, _, mask) in tqdm.tqdm(
            ...
            user_vec_batch = last_hidden[:, -1, :]                           # (B, D)

            for b in range(user_vec_batch.size(0)):
                user_vec = user_vec_batch[b]          # (D,)
                tgt = target[b]

                # ==== (1) 対象ユーザの複数興味 embedding を取得 ====
                user_interest_embs = all_user_embeddings.get(users_processed)
                if user_interest_embs is None:
                    users_processed += 1
                    continue
                # ==== (2) user_vec との内積でランキング ====
                # (num_interest,)
                sim_scores = torch.matmul(user_interest_embs, user_vec)

                topk_vals, topk_idx = torch.topk(sim_scores, args.k)
                topk_user_embs = user_interest_embs[topk_idx]  # (K, D)

                # ==== (3) 各 embedding で item score 計算 ====
                # (K, num_items)
                item_scores_multi = torch.matmul(topk_user_embs, item_embeddings.T)

                # ==== (4) 重複なしで統合（max pooling） ====
                # (num_items,)
                final_item_scores, _ = torch.max(item_scores_multi, dim=0)
