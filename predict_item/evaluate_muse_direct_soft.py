    ...
    with torch.no_grad():
        ...
        for batch_idx, (pos_emb, target, _, mask) in tqdm.tqdm(
            ...
            for b in range(user_vec_batch.size(0)):
                ...
                # ==== (3) 重みとして正規化 ====
                weights = torch.softmax(sim_scores, dim=0)            # (M,)

                # ==== (4) item score を加重平均で計算 ====
                # (M, num_items)
                item_scores_multi = torch.matmul(user_interest_embs, item_embeddings.T)

                # 重み付け
                weighted_item_scores = item_scores_multi * weights.unsqueeze(1)

                # (num_items,)
                final_item_scores = weighted_item_scores.sum(dim=0)
                # ---- top-N 推薦 ----
