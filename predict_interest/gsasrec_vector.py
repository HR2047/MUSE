...
class GSASRecVec(torch.nn.Module):
    ...
    def forward(self, input_embeddings, mask=None):
        ...
        # print("input min:", input.min().item(), "input max:", input.max().item())
        # print("embedding vocab size:", self.item_embedding.num_embeddings)
        
        bs, seq_len, _ = input_embeddings.size()

        # seq = self.item_embedding(input.long()) #変更予定
        # mask = (input != self.num_items + 1).float().unsqueeze(-1)
        
        positions = torch.arange(seq_len).unsqueeze(0).repeat(bs, 1).to(input_embeddings.device)
    ...
    def predict_next_embedding(self, input_embeddings, mask=None):
        with torch.no_grad():
            seq_emb, _ = self.forward(input_embeddings, mask)
            return seq_emb[:, -1, :]
