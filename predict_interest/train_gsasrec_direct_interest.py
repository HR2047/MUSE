...
for pos_emb, neg_emb, pos_mask in train_dataloader:
    step += 1

    # to device
    pos_emb = pos_emb.to(device)       # (B,T,D)
    neg_emb = neg_emb.to(device)       # (B,T,N,D)
    pos_mask = pos_mask.to(device)     # (B,T)
    
    # input(最後の時刻除く)
    model_input = pos_emb[:, :-1, :]                        # (B, T-1, D)
    mask_input  = pos_mask[:, :-1]                          # (B, T-1)
    last_hidden_state, _ = model(model_input, mask_input)   # (B, T-1, D)
    
    # 最初の時刻以外
    labels = pos_emb[:, 1:, :]          # (B, T-1, D)
    negatives = neg_emb[:, 1:, :, :]    # (B, T-1, N, D)
    
    # 結合(0:positive, 1~n:negative) (B, T-1, 1+N, D)
    pos_neg_concat = torch.cat([labels.unsqueeze(2), negatives], dim=2)
    
    # output_embeddings = model.get_output_embeddings()
    # pos_neg_embeddings = output_embeddings(target)
    
    # mask = (model_input != num_items + 1).float()
    logits = torch.einsum('bse, bsne -> bsn', last_hidden_state, pos_neg_concat)
    gt = torch.zeros_like(logits)
    gt[:, :, 0] = 1

    alpha = config.negs_per_pos / (config.num_items - 1)
    t = config.gbce_t 
    ...
    loss_per_element = torch.nn.functional.binary_cross_entropy_with_logits(logits, gt, reduction='none').mean(-1)*pos_mask[:, 1:]
    loss = loss_per_element.sum() / pos_mask[:, 1:].sum()
    loss.backward()
    optimiser.step()
    optimiser.zero_grad()
    loss_sum += loss.item()
